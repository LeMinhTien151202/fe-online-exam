...
export const usePart2 = () => {
  ...
  // Luyện theo phần = đề PART_PRACTICE (skill 5, part 2 — RECORD theo bộ).
  const { examId, examDetail, isLoading } = usePartPracticeExam(5, 2);
  const sets = useMemo(() => {
    if (!examDetail) return [];
    const part = flattenSpeakingExam(examDetail).find((p) => p.partNumber === 2);
  ...
  const totalSubQuestions = currentSet.questions.length;
  const activeQuestion = currentSet.questions[currentSubIndex - 1] ?? { id: 0, questionText: '', sampleAnswers: [] };
  ...
  // Chấm ngay CẢ BỘ hiện tại (P2: 1 bộ = 1 bản ghi = mảng URL theo thứ tự câu con).
  const currentGradeKey = raw?.questionId != null ? `p2-set${safeSet}` : '';
