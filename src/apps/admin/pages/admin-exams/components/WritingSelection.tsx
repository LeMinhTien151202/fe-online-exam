...
type WritingSlot = Partial<IBankQuestion> & { _slotIndex: number; isPlaceholder?: boolean };
...
const WritingSelection: React.FC<Props> = ({
    selectedQuestions,
    handleAddQuestion,
    ...
    // Writing P1: 5 câu独立 (mỗi câu 1 bản ghi); P2/P3/P4: 1 bản ghi gộp
    const slotCount = (p: string) => p === 'Part 1' ? 5 : 1;
    ...
    return (
        <Row gutter={16}>
            ...
            <Col xs={24} lg={12}>
                <Card
                    title={`Ngân hàng Writing ${mode === 'partial' ? targetPart : ''}`}
                    bordered={false}
                    extra={
                        <Input
                ...
                <Card
                    title={
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span>{mode === 'partial' ? `Cấu hình ${targetPart}` : 'Cấu trúc bộ đề Writing'}</span>
                        </div>
                    }
                    bordered={false}
                >
                    <div style={{ maxHeight: '640px', overflowY: 'auto', paddingRight: '4px' }}>
                        {displayParts.map(p => {
                            const partQs = selectedQuestions.filter(q => q && q.part === p && q.type === 'Writing');
                            const totalSlots = slotCount(p);

                            return (
                                <div key={p} style={{ marginBottom: '16px', border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
                                    ...
                                    <div style={{ padding: '8px' }}>
                                        {totalSlots === 1 ? (
                                            ...
                                            <List
                                                ...
                                                rowKey={(item: WritingSlot) => item?.id || item?.key || `slot-${item?._slotIndex}`}
                                                renderItem={(item: WritingSlot) => (
...
export default WritingSelection;
