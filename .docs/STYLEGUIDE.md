...
1. **Styled-components (Ưu tiên số 1)**:
   - Là lựa chọn mặc định cho MỌI style tùy chỉnh: layout, spacing, flexbox/grid, typography, và customize sâu Component của Ant Design.
   - Luôn tách biệt styled-components ra file riêng (`*.styled.ts`), không gộp chung với file giao diện.
   - Import chuẩn mực: `import * as S from './styled'`, dùng dưới dạng `<S.Wrapper>`.
2. **Utility-First (Tailwind CSS)**:
   - Chỉ dùng hạn chế cho các trường hợp cực kỳ đơn giản, nhanh gọn, không đáng tạo styled component riêng.
   - Không lạm dụng thay thế cho styled-components.
3. **Sass/SCSS**:
